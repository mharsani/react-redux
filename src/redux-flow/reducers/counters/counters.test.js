...
it('Should decrement an other counter', () => {
    const before = deepFreeze([1, 1, 8])
    const action = deepFreeze({ type: DECREMENT, index: 2})
    const after = [1, 1, 7]
    expect(counters(before, action)).to.be.deep.equal(after)
})

it('Should return same state if action in unknown', () => {
    const before = deepFreeze([1, 1, 1])
    const action = deepFreeze({ type: 'UNKNOWN'})
    const after = [1, 1, 1]
    expect(counters(before, action)).to.be.deep.equal(after)
})

it('Should return same state if last in undefined', () => {
